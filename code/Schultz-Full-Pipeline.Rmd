...
for (i in 1:nrow(map_data_sub)) {
  ...
  Stations <- map_data_sub$Station[i]
  Species <- map_data_sub$Var2[[i]]
  values <- map_data_sub$freq[[i]]
  X <- map_data_sub$newX[[i]]
  Y <- map_data_sub$newY[[i]]
  
  # Draw the pie chart for the current location
  draw.pie(values, x=X, y=Y, radius = 20, labels = Species, location = Stations)
}
