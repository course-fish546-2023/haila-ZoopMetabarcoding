...
## NMDS
My code for this section is working in rstudio, but when I go to knit the document, I am getting an error in the NMDS step. I have added screenshots of the plots that result from the code in rstudio.

Prep table for NMDS
```{r}
# transpose
read_table_transposed <- as.data.frame(transpose(read_table_summed))
#remove first row
read_table_transposed <- as.data.frame(read_table_transposed[-1,]) 
#convert to matrix
...
```{r, eval=TRUE}
#export file names
env <- as.data.frame(colnames(read_table_summed))
env <- as.data.frame(env[-1,])
#change column name
colnames(env)[1] <- "file"
#create year column
...
```
![](/Users/hailaschultz/GitHub/haila-ZoopMetabarcoding/screenshots/station_plot.png)
...
```
![](/Users/hailaschultz/GitHub/haila-ZoopMetabarcoding/screenshots/year_plot.png)
...
```
![](/Users/hailaschultz/GitHub/haila-ZoopMetabarcoding/screenshots/month_plot.png)
...
```
![](/Users/hailaschultz/GitHub/haila-ZoopMetabarcoding/screenshots/run_plot.png)

## Diversity Indices
Here I am calculating the shannon's diversity index.
```{r, eval=TRUE}
#convert to matrix
